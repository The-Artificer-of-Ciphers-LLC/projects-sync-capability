  ...
  for (const phase of phasesData.phases) {
    try {
      ...
      if (existing === null || existing === undefined) {
        // Create path
        const created = github.createIssue({ title, body, labels: [label] });
        receipt.created.push({ phase: phase.number, number: created.number });
      } else {
        // Update path — always sync body+label
        github.updateIssue(existing.number, { body, labels: [label] });
...
function sync({ cwd, repo, gsdExec, ghExec, board }) {
  const phasesData = loadPhases({ cwd, exec: gsdExec });
  ...
  const github = createGitHubClient({ repo, exec: ghExec });

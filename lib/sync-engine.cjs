...
const { phaseMarker, statusForPhase, LABELS } = require('./markers.cjs');
...
function runSync({ phasesData, github, milestoneTitle }) {
  ...
  // --- Ensure labels exist (once, before phase loop) -----------------------
  // `gh issue create --label <x>` FAILS if the label does not already exist, so
  // every gsd:* status label must be pre-created or the whole create path errors
  // on a fresh repository. Non-fatal: a failure here surfaces as per-phase errors
  // below, exactly as it would without this pass.
  try {
    github.ensureLabels(Object.values(LABELS));
  } catch (err) {
    receipt.errors.push({ phase: null, kind: 'ensureLabels', error: String(err.message ?? err) });
  }

  // --- Ensure milestone (once, before phase loop) --------------------------
  // Track success: if the milestone cannot be ensured we must NOT pass its title
  // to createIssue/updateIssue, because `gh issue create --milestone <t>` fails
  // when <t> does not resolve — which would doom every issue in the run. Degrade
  // to a milestone-less sync instead.
  ...
  const milestoneArg = milestoneOk ? milestoneTitle : undefined;
  ...
  for (const phase of phasesData.phases) {
    try {
      ...
      if (existing === null || existing === undefined) {
        // Create path — F3: pass the milestone (only when ensured) so gh assigns it.
        ...
        const created = github.createIssue({ title, body, labels: [label], milestone: milestoneArg });
        receipt.created.push({ phase: phase.number, number: created.number });

        // Close-on-create: a freshly-created issue defaults to open. If the phase
        // is already complete (state === 'closed'), transition it now — otherwise a
        // completed phase would be mirrored as a perpetually-open issue.
        if (state === 'closed') {
          github.setIssueState(created.number, state);
          receipt.closed.push({ phase: phase.number, number: created.number });
        }
      } else {
        // Update path — always sync body+label; F3: keep milestone in sync (only when ensured)
        github.updateIssue(existing.number, { body, labels: [label], milestone: milestoneArg });

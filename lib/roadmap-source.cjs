...
/**
 * Default exec implementation — calls `gsd-tools` via Node's execFileSync.
 * Replace this with your own function in tests.
 *
 * @param {string[]} argv
 * @returns {string}
 */
function realExec(argv) {
  // gsd-tools is the CLI entry point for the gsd-core capability.
  // We invoke it via `node` to ensure cross-platform compatibility.
 ...
 * }>}
 */
async function loadPhases({ cwd, exec = realExec }) {
